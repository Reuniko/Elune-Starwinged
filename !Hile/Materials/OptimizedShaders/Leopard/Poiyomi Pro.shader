Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/6d68d538548d24d46ab2388b638593ba"
{
	Properties
	{
		...
		[HideInInspector] m_start_MainHueShift_Leopard ("Color Adjust--{reference_property:_MainColorAdjustToggle,button_help:{text:Tutorial,action:{type:URL,data:https://www.poiyomi.com/color-and-normals/color-adjust},hover:Documentation}}", Float) = 0
		...
		_Saturation_Leopard ("Saturation", Range(-1, 10)) = 0
		...
		_MainHueShift_Leopard ("Hue Shift--{condition_showS:(_MainHueShiftToggle==1)}", Range(0, 1)) = 0
		...
		[HideInInspector] m_end_MainHueShift_Leopard ("Hue Shift", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			float _MainHueShift_Leopard;
			float _MainHueShiftSpeed;
			float _Saturation_Leopard;
			...
			{
				float2 st = frac(poiMesh.uv[(0.0 /*_GlitterUV*/)] + float4(0,0,0,0).xy * _Time.x) * (3000.0 /*_GlitterFrequency*/);
				...
				switch((0.0 /*_GlitterMode*/))
				{
					...
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, (14.5 /*_GlitterAngleRange*/), (10.0 /*_GlitterSpeed*/));
					}
					else
					{
						randomRotation = randomFloat3Range(randoPoint, (14.5 /*_GlitterAngleRange*/));
					}
					float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
					finalGlitter = lerp(0, _GlitterMinBrightness * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.8 /*_GlitterBias*/)), poiCam.viewDir)), (506.0 /*_GlitterContrast*/)), 0);
					...
					finalGlitter = max(_GlitterMinBrightness * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * (0.08 /*_GlitterCenterSize*/) * 10));
					break;
					case 2:
					if ((10.0 /*_GlitterSpeed*/) > 0)
					{
					...
					finalGlitter = lerp(0, _GlitterMinBrightness * glitterAlpha, glitterAlpha) + max(pow(specAngle, (506.0 /*_GlitterContrast*/)), 0);
			...
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min((0.0 /*_RimEnviroSharpness*/), (0.615 /*_RimEnviroWidth*/)), (0.615 /*_RimEnviroWidth*/), poiCam.vDotN));
				...
				#endif
				float3 envRimCol = lerp(0, max(0, (enviroRimColor - (0.0 /*_RimEnviroMinBrightness*/)) * poiFragData.baseColor), enviroRimAlpha).rgb * enviroMask * (0.712 /*_RimEnviroIntensity*/);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					float shift = _MainHueShift_Leopard;
					#ifdef POI_AUDIOLINK
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -(_Saturation_Leopard) * hueShiftAlpha.b);
