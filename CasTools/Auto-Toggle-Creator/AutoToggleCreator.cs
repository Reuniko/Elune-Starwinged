    ...
    public class AutoToggleCreator : EditorWindow
    {
        ...
        private void CreateClips()
        {
            foreach (var toggle in Toggles)
            {
                ...
                {
                    float firstValue = toggle.objectOnStates[i] ? 0 : 1;
                    float secondValue = toggle.objectOffStates[i] ? 0 : 1;
                    ...
                    toggleClipOn.SetCurve(
                        ...
                        typeof(GameObject),
                        "m_IsActive",
                        new AnimationCurve(new Keyframe(0.0f, firstValue, 0, 0))
                    );
                    toggleClipOff.SetCurve(
                        GetGameObjectPath(toggle.toggleObject[i].transform).Substring(GetGameObjectPath(myAnimator.transform).Length + 1),
                        typeof(GameObject),
                        "m_IsActive",
                        new AnimationCurve(new Keyframe(0.0f, secondValue, 0, 0))
                    );
                }

                for (int i = 0; i < toggle.shapekeyName.Count; i++) // For each shapekey add 
                {
                    float firstValue = toggle.shapekeyOffStates[i];
                    float secondValue = toggle.shapekeyOnStates[i];
        ...
        private void ApplyToAnimator()
        {
            if (CombineToggles)
            {
                ...
                sm.states[0].state.motion = (Motion)AssetDatabase.LoadAssetAtPath("Assets/CasTools/Auto-Toggle-Creator/IDLE.anim", typeof(Motion));
